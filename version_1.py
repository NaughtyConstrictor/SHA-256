...
def sha_256(message: bytes | str) -> bytes:
    ...
    K = _find_K(L)
    # K + 1 + (BYTE_2_BITS - 1) = K + BYTE_2_BITS
    K = (1 << K).to_bytes((K + BYTE_2_BITS) // BYTE_2_BITS, byteorder="big")
    ...
    for i in range(0, len(message), CHUNK_SIZE):
        ...
        # copying the first 16 bytes of chunk into w as 32-bit (4 bytes) words
        # by concatenating the binary representation of each byte
        # and converting back to int
        w[:16] = [
            int(
                f"{chunk[i]:08b}{chunk[i + 1]:08b}{chunk[i + 2]:08b}{chunk[i + 3]:08b}",
                base=2
            )
    ...
    # transform each hash byte into a 32-bit integer
    # concatenate the binary representation of each integer
    # convert the binary string into a bytes object
    return int("".join(
        bin(value)[2:].zfill(WORD_SIZE)
        for value in hash_values
        ), base=2).to_bytes(WORD_SIZE, byteorder="big")
...
def right_rotate(number: int, offset: int) -> int:
    number = bin(number)[2:].zfill(WORD_SIZE)
    return int(number[-offset:] + number[:WORD_SIZE - offset], base=2)
